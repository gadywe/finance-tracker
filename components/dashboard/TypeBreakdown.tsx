...
export default function TypeBreakdown({ incomeJobs }: Props) {
  ...
  return (
    <div style={{ background: 'var(--bg2)', border: '1px solid var(--border)' }} className="rounded-xl p-4 space-y-3">
      ...
      {INCOME_TYPES.map(({ value, label, color }) => {
        const paid = incomeJobs.filter((j) => j.type === value && j.status === 'paid').reduce((s, j) => s + j.amount, 0)
        const expected = incomeJobs.filter((j) => j.type === value && j.status === 'expected').reduce((s, j) => s + j.amount, 0)
